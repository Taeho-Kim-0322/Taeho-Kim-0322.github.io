...
### 피라미드 구조의 글을 작성하기 위해선, 3가지 규칙을 점검해야 한다
  ...
  2. 그룹 내의 메시지는 항상 동일한 종류여야 한다
    1. 메시지지의 동일성, 제대로 그루핑 되었는지의 여부는 메시지를 하나의 복수명으로 묶을 수 있는지 확인해본다!
  3. 그룹 내의 메시지는 항상 논리적 순서(4가지 방법)으로 배열되어야 한다.
    1. 1) 연역적 순서 (대전제, 소전제, 결론)
    2. 2) 시간적 순서 (첫 번째, 두 번째, 세 번째)
    3. 3) 구조적 순서 (보스턴, 뉴욕, 워싱턴)
    4. 4) 비교적 순서 (첫 번째 중요한 점, 두 번째 중요한 점)
    5. 연역적 추론, 인과관계, 전체를 부분으로 나누기, 유형별로 나누기 등 4가지 작업이 뇌가 수행할 수 있는 분석활동의 전부이다!
